...
exports.verifyCode = exports.sendVerifyCode = void 0;
...
exports.sendVerifyCode = (0, https_1.onCall)({
    region: "europe-west1",
    cors: true,
}, async (request) => {
    ...
    if (!email)
        throw new https_1.HttpsError("failed-precondition", "E-posta bulunamadı.");
    ...
    if (lastSentAt && nowMs() - lastSentAt < 45000) {
        throw new https_1.HttpsError("resource-exhausted", "Çok hızlı tekrar denendi. 1 dk bekle.");
    }
    const code = genCode();
    ...
    <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #eee4d2;border-radius:20px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.08);">
      ...
      <div style="padding:28px 24px 18px;">
        ...
        <p style="margin:0 0 24px;font-size:15px;line-height:1.8;color:#4b5563;">
          6nci Kuyumculuk hesabınız için e-posta doğrulama işlemi başlatıldı.
...
`,
    ...
    firebase_functions_1.logger.info("verify code sent", { uid, email });
    return { ok: true };
});
exports.verifyCode = (0, https_1.onCall)({ region: "europe-west1", cors: true }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError("unauthenticated", "Giriş gerekli.");
    ...
    if (!verifySnap.exists) {
        throw new https_1.HttpsError("not-found", "Kod bulunamadı. Önce kod gönder.");
    }
    ...
    if (!expiresAt || nowMs() > expiresAt) {
        throw new https_1.HttpsError("deadline-exceeded", "Kod süresi dolmuş. Yeniden gönder.");
    }
    if (attempts >= 7) {
        throw new https_1.HttpsError("permission-denied", "Çok fazla deneme. Yeni kod iste.");
    }
    if (code !== stored) {
    ...
    if (!email) {
        throw new https_1.HttpsError("failed-precondition", "Doğrulanan kullanıcı için e-posta bulunamadı.");
    }
    ...
    firebase_functions_1.logger.info("verify code success + user ensured", { uid, email });
    return { ok: true };
});
