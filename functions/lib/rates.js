...
exports.updateRatesScheduler = exports.updateRatesHttp = void 0;
...
// ---- Params / Secrets ----
const RATES_PROVIDER = (0, params_1.defineString)("RATES_PROVIDER", { default: "demo" }); // "demo" | "harem"
const RAPIDAPI_KEY = (0, params_1.defineSecret)("RAPIDAPI_KEY");
const RATES_REFRESH_SECRET = (0, params_1.defineSecret)("RATES_REFRESH_SECRET");
const RAPIDAPI_HOST = (0, params_1.defineString)("RAPIDAPI_HOST", {
    default: "harem-altin-live-gold-price-data.p.rapidapi.com",
});
const HAREM_ENDPOINT = (0, params_1.defineString)("HAREM_ENDPOINT", {
...
function intervalToMs(value, unit = "hour") {
...
function isDue(lastRunAt, value, unit = "hour") {
...
function normalizeToRateItems(list) {
    const out = [];
    for (const x of list) {
        const code = s(x?.code ??
            x?.symbol ??
...
async function fetchHaremRates(opts) {
    ...
    firebase_functions_1.logger.info("Harem fetch", { host, url });
    ...
    firebase_functions_1.logger.info("Harem response received", {
        status: r.status,
        ok: r.ok,
        statusText: r.statusText,
    });
    const text = await r.text().catch(() => "");
    firebase_functions_1.logger.info("Harem raw response", {
        length: text.length,
    ...
    firebase_functions_1.logger.info("Harem parsed keys", {
        keys: data && typeof data === "object" ? Object.keys(data).slice(0, 20) : [],
    });
    const list = pickList(data);
    if (!list) {
        firebase_functions_1.logger.warn("Harem response shape unexpected", { sample: data });
        throw new Error("Harem response shape unexpected.");
    }
    const out = normalizeToRateItems(list);
    firebase_functions_1.logger.info("Harem normalized item count", {
        count: out.length,
    });
    if (!out.length) {
        firebase_functions_1.logger.warn("Harem normalize produced empty items", { sample: data });
...
function parseTcmbNum(raw) {
    if (!raw)
        return 0;
    const n = parseFloat(raw.trim());
    return Number.isFinite(n) ? n : 0;
}
async function fetchTcmbForexRates() {
    try {
        ...
        if (!r.ok) {
            firebase_functions_1.logger.warn("TCMB fetch failed", { status: r.status });
            return [];
        }
        ...
        firebase_functions_1.logger.info("TCMB forex rates fetched", { count: items.length });
        return items;
    }
    catch (e) {
        firebase_functions_1.logger.warn("TCMB forex fetch error", { message: e?.message || "unknown" });
        return [];
    }
}
// ---------- Provider ----------
async function fetchProviderRates(params) {
    const provider = (RATES_PROVIDER.value() || "demo").toLowerCase();
    if (provider === "demo")
        return fetchDemoRates();
    const key = s(params.rapidKey);
    if (!key) {
        firebase_functions_1.logger.warn("RATES_PROVIDER=harem ama RAPIDAPI_KEY yok -> demo fallback");
        return fetchDemoRates();
    }
...
// ---------- Write latest ----------
async function updateRatesLatest(params) {
    ...
    firebase_functions_1.logger.info("Provider result", {
        provider,
    ...
    const normalized = (mergedItems || [])
        ...
        buy: toNum(x.buy, 0),
        sell: toNum(x.sell, 0),
        change: typeof x.change === "undefined"
            ? null
            : x.change === null
                ? null
                : toNum(x.change, 0),
    ...
    firebase_functions_1.logger.info("Writing rates/latest", {
        docPath: DOC_PATH,
    ...
    await db.doc(DOC_PATH).set({
        ...
        fetchedAtTs: firestore_1.FieldValue.serverTimestamp(),
        count: normalized.length,
        items: normalized,
        itemsMap,
    }, { merge: true });
    firebase_functions_1.logger.info("rates/latest write success", {
        provider,
        normalizedCount: normalized.length,
        fetchedAtIso,
    });
...
async function shouldRunRatesUpdate() {
    ...
    const globalUnit = rateSettings?.refreshIntervalUnit === "day" ? "day" : "hour";
    const globalLastRunAt = rateSettings?.lastFetchedAt || rateSettings?.lastRunAt || null;
    if (globalEnabled) {
        if (globalMode !== "auto") {
            return {
                shouldRun: false,
    ...
    for (const d of catSnap.docs) {
        ...
        const unit = pricing?.refreshIntervalUnit === "day" ? "day" : "hour";
...
async function markRatesRunTimestamps() {
    ...
    await db.doc("site_options/rate_settings").set({
        ...
        lastFetchedAtTs: firestore_1.FieldValue.serverTimestamp(),
    ...
    for (const d of catSnap.docs) {
        ...
        if (!pricing?.enabled)
            continue;
        if (String(pricing?.refreshMode || "manual").toLowerCase() !== "auto")
            continue;
        jobs.push(d.ref.set({
            pricing: {
                ...pricing,
...
function getWeight(product) {
    return Math.max(0, toNum(product?.hasGram ??
        product?.gram ??
        product?.weightGram ??
        product?.weightGr ??
        0, 0));
}
function getRateValueFromLatest(ratesData, rateKey) {
    ...
    if (Array.isArray(items)) {
        const row = items.find((x) => normKey(x?.code || x?.name || "") === key);
...
function calcFinalPrice(product, rate, pricing) {
    const mode = String(product?.priceMode ?? "fixed").toLowerCase();
    const percent = Math.max(0, toNum(pricing?.pricePercent ?? product?.pricePercent ?? 0, 0));
    const fixedAdd = Math.max(0, toNum(pricing?.priceFixedAdd ?? product?.priceFixedAdd ?? 0, 0));
    // Override kontrolü
    const overrideEnabled = product?.priceOverrideEnabled === true ||
        String(product?.priceOverrideEnabled ?? "").toLowerCase() === "true";
    const override = toNum(product?.priceOverride ?? 0, 0);
    if (overrideEnabled && override > 0)
        return override;
    // Birinci motor
    let price1 = 0;
    if (mode === "fixed") {
        ...
        if (mode === "rate_plus")
            ...
            price1 = weight * rate * (1 + percent / 100);
        else if (mode === "weight_rate_plus_fixed")
            price1 = weight * rate * (1 + percent / 100) + fixedAdd;
        else
            price1 = toNum(product?.finalPrice ?? product?.price ?? 0, 0);
    }
    return price1;
...
function calcPrice2(product, ratesData) {
    if (!product?.price2Enabled)
        return 0;
    ...
    if (rate2 <= 0)
        return 0;
    if (mode2 === "rate_plus")
        return rate2 * (1 + percent2 / 100);
    if (mode2 === "rate_plus_fixed")
        return rate2 * (1 + percent2 / 100) + fixedAdd2;
    if (mode2 === "weight_rate")
        return hasGram2 * rate2;
    if (mode2 === "weight_rate_plus")
        return hasGram2 * rate2 * (1 + percent2 / 100);
    if (mode2 === "weight_rate_plus_fixed")
        return hasGram2 * rate2 * (1 + percent2 / 100) + fixedAdd2;
    return 0;
}
async function findProductsByCategoryId(categoryId, categorySlug) {
    const found = new Map();
...
async function applyPricingToAllDynamicProducts() {
    ...
    if (!ratesLatest) {
        firebase_functions_1.logger.warn("applyPricingToAllDynamicProducts: no rates data");
    ...
    for (const catDoc of catSnap.docs) {
        ...
        const categorySlug = String(catData?.slug || "")
            ...
            .toLowerCase();
        try {
            const products = await findProductsByCategoryId(categoryId, categorySlug);
            // Zaten işlenen ürünleri atla
            const newProducts = products.filter((d) => !processedIds.has(d.id));
            if (!newProducts.length) {
                firebase_functions_1.logger.info(`applyPricing: category ${categoryId} (${categorySlug}): no new products (${products.length} already processed)`);
                continue;
            }
            ...
            for (let i = 0; i < newProducts.length; i += BATCH_SIZE) {
                ...
                for (const prodDoc of chunk) {
                    ...
                    const rateKey = String(product?.priceRateCode || product?.rateKey || "GRAM_ALTIN").trim();
                    ...
                    batch.update(prodDoc.ref, {
                        ...
                        lastPriceAppliedAt: firestore_1.FieldValue.serverTimestamp(),
                        updatedAt: firestore_1.FieldValue.serverTimestamp(),
                    });
                }
                await batch.commit();
            ...
            firebase_functions_1.logger.info(`applyPricing: category ${categoryId}: ${newProducts.length} products updated (${products.length - newProducts.length} skipped)`);
        }
        catch (catErr) {
            const msg = `applyPricing: category ${categoryId} error: ${catErr?.message || "unknown"}`;
            firebase_functions_1.logger.error(msg);
            errors.push(msg);
            // Bir kategoride hata olursa diğerlerine devam et
        }
    }
    firebase_functions_1.logger.info("applyPricingToAllDynamicProducts complete", {
        totalUpdated,
...
// ---------- HTTP trigger ----------
exports.updateRatesHttp = (0, https_1.onRequest)({
    region: "europe-west1",
    cors: true,
    ...
    try {
        ...
        firebase_functions_1.logger.info("updateRatesHttp request", {
            method: req.method,
        ...
        if (!got || got !== RATES_REFRESH_SECRET.value()) {
            firebase_functions_1.logger.warn("updateRatesHttp unauthorized", {
                hasRefreshSecretHeader: !!got,
            });
        ...
        firebase_functions_1.logger.info("updateRatesHttp authorized, starting update", {
            provider: RATES_PROVIDER.value(),
        ...
        firebase_functions_1.logger.info("updateRatesHttp pricing applied", pricingResult);
        firebase_functions_1.logger.info("updateRatesHttp success", result);
        res.status(200).json({
            ok: true,
            ...result,
        ...
        firebase_functions_1.logger.error("updateRatesHttp error", {
...
// ---------- Scheduler ----------
exports.updateRatesScheduler = (0, scheduler_1.onSchedule)({
    region: "europe-west1",
    ...
    try {
        ...
        firebase_functions_1.logger.info("updateRatesScheduler decision", decision);
        if (!decision.shouldRun)
            return;
        if ((RATES_PROVIDER.value() || "demo").toLowerCase() === "demo") {
            firebase_functions_1.logger.info("Scheduler skipped: provider is demo");
            return;
        }
        const rapidKey = RAPIDAPI_KEY.value();
        const result = await updateRatesLatest({ rapidKey });
        ...
        // Ürün fiyatlarını da güncelle
        const pricingResult = await applyPricingToAllDynamicProducts();
        firebase_functions_1.logger.info("updateRatesScheduler pricing applied", pricingResult);
        firebase_functions_1.logger.info("updateRatesScheduler ok", result);
    }
    catch (e) {
        firebase_functions_1.logger.error("updateRatesScheduler error", {
            message: e?.message || "unknown",
            stack: e?.stack || "",
        });
    }
