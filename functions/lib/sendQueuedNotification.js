...
exports.sendQueuedNotification = void 0;
...
function buildFcmData(data, notificationId, url, image) {
    ...
    if (threadId)
        fcmData.threadId = threadId;
    if (supportThreadId)
        fcmData.supportThreadId = supportThreadId;
    if (messageId)
        fcmData.messageId = messageId;
    if (orderId)
        fcmData.orderId = orderId;
    if (refundId)
        fcmData.refundId = refundId;
    if (productId)
        fcmData.productId = productId;
    if (kind)
        fcmData.kind = kind;
    if (paymentLabel)
        fcmData.paymentLabel = paymentLabel;
    if (paymentMethod)
        fcmData.paymentMethod = paymentMethod;
    if (paymentProvider)
        fcmData.paymentProvider = paymentProvider;
    if (paymentStatus)
        fcmData.paymentStatus = paymentStatus;
    if (orderStatus)
        fcmData.orderStatus = orderStatus;
    if (totalTry)
        fcmData.totalTry = totalTry;
    if (image)
        fcmData.image = image;
    return fcmData;
}
exports.sendQueuedNotification = (0, firestore_1.onDocumentCreated)({
    region: "europe-west1",
    document: "notifications/{notificationId}",
}, async (event) => {
    firebase_functions_1.logger.info("STEP 0: trigger entered", {
        hasEventData: !!event.data,
        params: event.params || null,
    });
    const snap = event.data;
    if (!snap) {
        firebase_functions_1.logger.error("STEP X: event.data missing");
        return;
    }
    try {
        firebase_functions_1.logger.info("STEP 1: snapshot ok", {
        ...
        firebase_functions_1.logger.info("STEP 2: notification parsed", {
            notificationId,
            title,
        ...
        if (!title || !body) {
            firebase_functions_1.logger.error("STEP X: missing title/body", {
                notificationId,
                title,
                body,
            });
        ...
        firebase_functions_1.logger.info("STEP 3: setting processing status", { notificationId });
        await snap.ref.set({
            status: "processing",
        ...
        if (targetUserIds.length > 0) {
            ...
            firebase_functions_1.logger.info("STEP 4A: using explicit targetUserIds", {
                notificationId,
                targetCount: allowedUserIds.size,
            });
        }
        else if (targetRoles.length > 0 || !!targetPermission) {
            firebase_functions_1.logger.info("STEP 4B: resolving users by role/permission", {
                notificationId,
                targetRoles,
                targetPermission,
            });
            ...
            usersSnap.forEach((docSnap) => {
                ...
                if (targetRoles.length > 0 && !targetRoles.includes(role))
                    return;
                if (targetPermission) {
                    const hasPermission = role === "admin" || perms[targetPermission] === true;
                    if (!hasPermission)
                        return;
                }
                allowedUserIds.add(uid);
            });
            firebase_functions_1.logger.info("STEP 4C: resolved users by role/permission", {
                notificationId,
                allowedUserCount: allowedUserIds.size,
                allowedUserIds: Array.from(allowedUserIds),
            });
        }
        firebase_functions_1.logger.info("STEP 5: querying push_tokens collectionGroup", {
            notificationId,
        });
        const tokensSnap = await db.collectionGroup("push_tokens").get();
        firebase_functions_1.logger.info("STEP 6: query completed", {
            notificationId,
            docsCount: tokensSnap.size,
        });
        const tokenRows = tokensSnap.docs
            .map((docSnap) => {
        ...
        firebase_functions_1.logger.info("STEP 7: tokens normalized + filtered", {
            notificationId,
        ...
        if (!tokenRows.length) {
            firebase_functions_1.logger.warn("STEP 8: no active tokens found after filtering", {
                notificationId,
                targetUserIds: Array.from(allowedUserIds),
            });
            await snap.ref.set({
            ...
            firebase_functions_1.logger.info("STEP 9: completed with 0 send", { notificationId });
            return;
        }
        const chunks = [];
        ...
        firebase_functions_1.logger.info("STEP 10: chunks prepared", {
            notificationId,
        ...
        let success = 0;
        let failure = 0;
        const refsToDeactivate = [];
        for (const [chunkIndex, chunk] of chunks.entries()) {
            const tokens = chunk.map((x) => x.token);
            firebase_functions_1.logger.info("STEP 11: sending chunk", {
                notificationId,
                chunkIndex,
            ...
            if (seenInboxKeys.size > 0) {
                ...
                firebase_functions_1.logger.info("STEP 11B: inbox notifications written", {
                    notificationId,
                    count: seenInboxKeys.size,
                });
            }
            firebase_functions_1.logger.info("STEP 12: chunk result", {
                notificationId,
                chunkIndex,
            ...
            response.responses.forEach((item, idx) => {
                ...
                firebase_functions_1.logger.error("STEP 13: push send failure", {
                    notificationId,
                    chunkIndex,
                    tokenIndex: idx,
        ...
        firebase_functions_1.logger.info("STEP 14: all chunks completed", {
            notificationId,
            success,
            failure,
        ...
        if (refsToDeactivate.length) {
            ...
            firebase_functions_1.logger.info("STEP 15: invalid tokens deactivated", {
                notificationId,
        ...
        firebase_functions_1.logger.info("STEP 16: notification finished", {
            notificationId,
            success,
            failure,
        });
        ...
        firebase_functions_1.logger.error("STEP FATAL: sendQueuedNotification crashed", {
        ...
        try {
            ...
            firebase_functions_1.logger.error("STEP FATAL-2: failed to mark notification as failed", {
