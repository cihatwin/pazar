...
function getInitial(title: string) {
  return safeStr(title).slice(0, 1).toLocaleUpperCase("tr-TR") || "P";
}
      ...
      <div className={styles.head}>
        <div className={styles.brandSide}>
          <div className={styles.rocketBadge} aria-hidden="true">
            <span className={styles.rocket}>P.</span>
          </div>

          <div>
            <div className={styles.kicker}>
              {isTR ? "PAZAR KEŞİF MOTORU" : "PAZAR DISCOVERY ENGINE"}
            </div>
            <h2 className={styles.title}>
              {isTR ? "Aradığını bul" : "Find your next pick"}
            </h2>
          </div>
        </div>
      ...
      <div className={styles.body}>
        <form className={styles.searchCard} onSubmit={onSubmit}>
          ...
          <input
            ...
            placeholder={
              isTR
                ? "Ürün, marka, kategori veya mağaza ara..."
                : "Search products, brands, categories or stores..."
            }
            autoComplete="off"
          />
        ...
        <div className={styles.quickBar}>
          <button type="button" onClick={() => setQ("yeni")}>
            Yeni
          </button>
          <button type="button" onClick={() => setQ("teknoloji")}>
            Teknoloji
          </button>
          <button type="button" onClick={() => setQ("moda")}>
            Moda
          </button>
          <button type="button" onClick={() => setQ("ev yaşam")}>
            Ev & Yaşam
          </button>
        </div>
        ...
        {searchLoading ? (
          <div className={styles.stateCard}>
            <div className={styles.orbit}>
              <span className={styles.orbitCore}>P.</span>
          ...
          <div className={styles.hintCard}>
            ...
            <h3>{isTR ? "Yeni keşiflere hazır" : "Ready to discover"}</h3>
            <p>
              {isTR
                ? "Ürün, marka, kategori veya mağaza adını yaz. Pazarın tamamını senin için tarayalım."
                : "Type a product, brand, category or store name and explore the whole marketplace."}
            </p>
          </div>
        )}
      </div>
