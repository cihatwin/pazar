...
useEffect(() => {
  ...
  const resetNav: NavItem[] = [
    { label: { tr: "Ana sayfa", en: "Home" }, url: "/" },
    { label: { tr: "Mağaza", en: "Shop" }, url: "/shop" },
    { label: { tr: "Satıcı ol", en: "Sell with us" }, url: "/satici" },
  ];
  const navBase = CATALOG_RESET_MODE
    ? resetNav
    : Array.isArray(hdr?.nav) && hdr?.nav.length
      ? hdr.nav
      : fallbackNav;
  ...
  const brandLogoUrl = CATALOG_RESET_MODE ? "" : seg(brand?.logoUrl);
  const brandLink = safeUrl(brand?.logoLink || "/");
  const brandTitle = CATALOG_RESET_MODE
    ? { tr: "PAZAR.", en: "PAZAR." }
    : pickLocaleText(brand?.title, "PAZAR.", "PAZAR.");
  const brandMark = CATALOG_RESET_MODE ? "P." : seg(hdr?.brand?.markText || "6");
...
function removeCartLocal(id: string) {
    ...
    <>
      <header className={s.header}>
        <div className={s.inner}>
          ...
          <HeaderActions
            cartCount={CATALOG_RESET_MODE ? 0 : cartCount}
            wishCount={CATALOG_RESET_MODE ? 0 : favCount}
  ...
  <DrawerPanel
    ...
    tab={tab}
    setTab={setTab}
    cartItems={CATALOG_RESET_MODE ? [] : cartItems}
    favItems={CATALOG_RESET_MODE ? [] : favItems}
    favProductMap={favProductMap}
    rates={rates}
    productStockMap={productStockMap}
    cartCount={CATALOG_RESET_MODE ? 0 : cartCount}
    wishCount={CATALOG_RESET_MODE ? 0 : favCount}
    cartSubtotal={CATALOG_RESET_MODE ? 0 : cartSubtotal}
    money={money}
