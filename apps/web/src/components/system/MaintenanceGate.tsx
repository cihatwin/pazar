...
function ActiveMaintenanceGate({ children }: { children: ReactNode }) {
  ...
  const title = pickLT(
    loc,
    cfg?.title,
    "6’ncı Kuyumculuk yenileniyor",
    "6’ncı Jewelry is being updated"
  );
...
export default function MaintenanceGate({ children }: { children: ReactNode }) {
  if (CATALOG_RESET_MODE) return <>{children}</>;
  return <ActiveMaintenanceGate>{children}</ActiveMaintenanceGate>;
}
