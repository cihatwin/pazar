...
import {
  MARKETPLACE_CHAT_SETTINGS_DOC,
  MARKETPLACE_SUPPORT_COLLECTION,
  MARKETPLACE_SUPPORT_WORKSPACE,
  marketplaceAccountThreadId,
} from "@/lib/marketplaceSupport";
...
function getOrCreateSessionId() {
  const key = "pazar_chat_session_v1";
...
function getStoredThreadId(sessionId: string) {
  const key = `pazar_chat_thread_${sessionId}`;
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const snap = await getDoc(doc(db, "site_options", MARKETPLACE_CHAT_SETTINGS_DOC));
  ...
  if (!isAnonymousUser) {
    const tid = marketplaceAccountThreadId(uid);
    setThreadId(tid);
    setChatMode(false);
    setUnread(0);
    return;
...
useEffect(() => {
  if (CATALOG_RESET_MODE || !uid || isAnonymousUser) {
    setOrders([]);
    setSelectedOrderId("");
  ...
  useEffect(() => {
    ...
    const tref = doc(db, MARKETPLACE_SUPPORT_COLLECTION, threadId);
...
if (String(d?.workspace || "") !== MARKETPLACE_SUPPORT_WORKSPACE) {
  resetChatState(true);
  return;
}
...
setUnread(Number.isFinite(n) ? n : 0);
  ...
  useEffect(() => {
    ...
    const qy = query(
      collection(db, MARKETPLACE_SUPPORT_COLLECTION, threadId, "messages"),
    ...
    return onSnapshot(
      ...
      (snap) => {
       ...
       if (uid && threadId) {
  updateDoc(doc(db, MARKETPLACE_SUPPORT_COLLECTION, threadId), {
    unreadByUser: 0,
  }).catch((err) => {
...
async function ensureThread(): Promise<string> {
  ...
  if (!isAnonymousUser) {
    const tid = marketplaceAccountThreadId(uid);
    const ref = doc(db, MARKETPLACE_SUPPORT_COLLECTION, tid);
    ...
    await setDoc(
      ...
      {
        status: "open",
        workspace: MARKETPLACE_SUPPORT_WORKSPACE,
        uid,
        sessionId,
  ...
  if (currentThreadId) {
    try {
      const currentRef = doc(db, MARKETPLACE_SUPPORT_COLLECTION, currentThreadId);
      ...
      if (currentSnap.exists()) {
        ...
        if (String(d?.workspace || "") === MARKETPLACE_SUPPORT_WORKSPACE && (!ownerUid || ownerUid === String(uid))) {
  ...
  if (stored) {
    try {
      const storedRef = doc(db, MARKETPLACE_SUPPORT_COLLECTION, stored);
  ...
  const ref = await addDoc(collection(db, MARKETPLACE_SUPPORT_COLLECTION), {
    status: "open",
    workspace: MARKETPLACE_SUPPORT_WORKSPACE,
    createdAt: serverTimestamp(),
    updatedAt: serverTimestamp(),
...
function attachSelectedOrderToMessage() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const threadRef = doc(db, MARKETPLACE_SUPPORT_COLLECTION, tid);
...
email: isAnonymousUser
  ...
  { merge: true }
      ...
      await addDoc(collection(db, MARKETPLACE_SUPPORT_COLLECTION, tid, "messages"), {
        role: "user",
        sender: "user",
        senderType: "user",
        text: clean,
...
>
  <MessageCircle className={styles.bubbleIcon} aria-hidden="true" strokeWidth={2.2} />
                  ...
                  <div className={styles.grid2}>
                    ...
                    <label className={styles.field}>
                      ...
                      <input
                        ...
                        placeholder={loc === "en" ? "Phone number" : "05.."}
                      />
                    </label>
                  </div>
              ...
              {!hasChat ? (
                <div className={styles.welcome}>
                  <div className={styles.welcomeBubble}>
                    <b>{loc === "en" ? "Hello 👋" : "Merhaba 👋"}</b>
                    <br />
                    {isAnonymousUser
                      ? loc === "en"
                        ? "Tell us what you need. Live support starts with your first message."
                        : "İstersen yaz, hemen yardımcı olayım. İlk mesajdan sonra canlı sohbet başlar."
                      : loc === "en"
                        ? "Type your message and continue with live support."
