...
const QUICK_REPLIES: QuickReply[] = [
  ...
  },
  {
    id: "qr_3",
    label: "Fiyat bilgisi",
    text: "Merhaba, ürünün güncel fiyat ve seçeneklerini kontrol edebilirim. İlgilendiğiniz ürünü paylaşır mısınız?",
  },
  {
    id: "qr_4",
    label: "Stok kontrol",
...
function getThreadPreview(r: ThreadRow) {
  ...
  if (!text) return "Henüz son mesaj özeti yok. Konuşma detayını açabilirsin.";
...
function AdminSupportPageInner() {
  ...
  useEffect(() => {
    const qy = query(collection(db, MARKETPLACE_SUPPORT_COLLECTION), where("workspace", "==", MARKETPLACE_SUPPORT_WORKSPACE));
    return onSnapshot(
      qy,
      (snap) => {
        ...
        setRows(list.sort((a,b)=>tsMs(b.lastMessageAt || b.updatedAt)-tsMs(a.lastMessageAt || a.updatedAt)));
      },
      () => setRows([])
    );
  }, [db]);
  ...
  async function toggleThreadStatus(row: ThreadRow) {
    ...
    try {
      await updateDoc(doc(db, MARKETPLACE_SUPPORT_COLLECTION, row.id), {
        status: nextStatus,
        updatedAt: serverTimestamp(),
      });
      setNote(nextStatus === "closed" ? "Konuşma kapatıldı." : "Konuşma tekrar açıldı.");
    } catch (error) {
      console.error("support thread status update error:", error);
      setNote("Konuşma durumu güncellenemedi.");
    } finally {
      setBusyId("");
    }
  }
  ...
  async function sendQuickReply(row: ThreadRow, reply: QuickReply) {
    ...
    try {
      await addDoc(collection(db, MARKETPLACE_SUPPORT_COLLECTION, row.id, "messages"), {
        role: "admin",
        sender: "admin",
        senderType: "admin",
        text: reply.text,
  ...
  async function handleDeleteThread(row: ThreadRow) {
    ...
    try {
      await deleteDoc(doc(db, MARKETPLACE_SUPPORT_COLLECTION, row.id));
