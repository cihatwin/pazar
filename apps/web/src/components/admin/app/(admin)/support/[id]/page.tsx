...
const QUICK_REPLIES: QuickReply[] = [
  ...
  },
  {
    id: "q2",
    label: "Fiyat bilgisi",
    text: "Merhaba, ürünün güncel fiyatını ve seçeneklerini kontrol edebilirim. İlgilendiğiniz ürünü paylaşırsanız net bilgi verebilirim.",
  },
  {
    id: "q3",
    label: "Stok kontrol",
...
function AdminSupportDetailInner({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (!threadId) return;
    const ref = doc(db, MARKETPLACE_SUPPORT_COLLECTION, threadId);
    ...
    return onSnapshot(
      ...
      (snap) => {
        ...
        setThread(String(data?.workspace || "") === MARKETPLACE_SUPPORT_WORKSPACE ? { id: snap.id, ...data } : null);
      },
      () => setThread(null)
    );
  }, [db, threadId]);
  ...
  async function sendAdminMessage(customText?: string) {
    ...
    try {
      await addDoc(collection(db, MARKETPLACE_SUPPORT_COLLECTION, threadId, "messages"), {
        role: "admin",
        sender: "admin",
        senderType: "admin",
        text: clean,
      ...
      await updateDoc(doc(db, MARKETPLACE_SUPPORT_COLLECTION, threadId), {
        updatedAt: serverTimestamp(),
  ...
  async function deleteThread() {
    ...
    setNote("");
    try {
      await deleteDoc(doc(db, MARKETPLACE_SUPPORT_COLLECTION, threadId));
        ...
        <section className={styles.chatCard}>
          <div className={styles.chatHead}>
            <div>
              ...
              <div className={styles.chatSub}>PAZAR. canlı kanal • {threadId}</div>
            </div>
        ...
        <aside className={styles.sideCard}>
          ...
          <div className={styles.sideBlock}>
            ...
            <div className={styles.note}>
              Bu panel yalnızca <b>PAZAR. Marketplace</b> çalışma alanındaki canlı konuşmaları gösterir.
