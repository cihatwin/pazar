...
function AdminSystemPageInner() {
  ...
  useEffect(() => {
    ...
    if (CATALOG_RESET_MODE) {
      ...
      unsubs.push(
        onSnapshot(
          query(collection(db, MARKETPLACE_SUPPORT_COLLECTION), where("workspace", "==", MARKETPLACE_SUPPORT_WORKSPACE), limit(80)),
          (snap) => {
            const rows = snap.docs.map((d) => ({ id: d.id, ...(d.data() as any) }));
            rows.sort((a:any,b:any)=>tsMs(b.lastMessageAt || b.updatedAt)-tsMs(a.lastMessageAt || a.updatedAt));
            setThreads(rows);
            setLoading(false);
          },
          () => { setThreads([]); setLoading(false); }
        )
      );
