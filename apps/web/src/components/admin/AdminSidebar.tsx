...
import { MARKETPLACE_SUPPORT_COLLECTION, MARKETPLACE_SUPPORT_WORKSPACE } from "@/lib/marketplaceSupport";
...
const items: NavItem[] = [
  ...
  { section: "Katalog", href: "/admin/seller-products", label: "Ürün Onayları", icon: "✓", permission: "users_admin", tone: "emerald" },
  ...
  { section: "Satış", href: "/admin/sellers", label: "Satıcılar", icon: "◇", permission: "users_admin", tone: "violet" },
  ...
  { section: "Ayarlar", href: "/admin/altinci-kuyumculuk", label: "Hakkımızda", icon: "⚑", permission: "home_settings", tone: "emerald" },
...
function getRoleLabel(role: ViewRole) {
  if (role === "admin") return "Süper Admin";
...
useEffect(() => {
  ...
  if (CATALOG_RESET_MODE) {
    setCounts({ orders:0, refunds:0, reviews:0, support:0, stock:0, notifications:0, campaigns:0, appointments:0 });
    const unsubSupport = onSnapshot(
      query(collection(db, MARKETPLACE_SUPPORT_COLLECTION), where("workspace", "==", MARKETPLACE_SUPPORT_WORKSPACE), limit(100)),
      (snap) => {
        const unread = snap.docs.filter((item) => {
          const data = item.data() as any;
          return String(data.status || "open") !== "closed" && Number(data.unreadByAdmin || 0) > 0;
        }).length;
        setCounts((prev) => ({ ...prev, support: unread }));
      },
  ...
  const visibleItems = useMemo(() => {
    ...
    return items.filter((item) => hasPermission(item, userState.role, userState.permissions, userState.isActive));
  }, [userState]);
    ...
    <aside className={s.wrap}>
      ...
      <div className={s.brand}>
        ...
        <div className={s.brandText}>
          <div className={s.name}>PAZAR.</div>
          <div className={s.meta}>Marketplace OS</div>
        </div>
      </div>
      ...
      <div className={s.footerCard}>
        <div className={s.footerTitle}>
          {userState.isActive ? "PAZAR. sistemi aktif" : "Kullanıcı pasif"}
        </div>
        <div className={s.footerMeta}>v3 • Çok satıcılı pazar yönetimi</div>
      </div>
    </aside>
  );
}
