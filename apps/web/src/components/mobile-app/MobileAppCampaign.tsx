...
export const APP_STORE_URL = "https://apps.apple.com/tr/app/bizim-6nc%C4%B1-kuyumculuk/id6760553574?l=tr";
const DISMISS_KEY = "nci_app_launch_ad_until_v1";
const SEVEN_DAYS = 7 * 24 * 60 * 60 * 1000;

function AppIcon({ size = 54 }: { size?: number }) {
  return <span className={styles.appIcon} style={{ width: size, height: size, borderRadius: "50%", border: "2px solid rgba(218,185,91,.62)", boxShadow: "0 10px 30px rgba(0,0,0,.2),0 0 0 4px rgba(218,185,91,.08)" }}><img src="/1%201.png" alt="Bizim 6'ncı Kuyumculuk uygulaması" /></span>;
}

function StoreButton({ compact = false, source = "unknown" }: { compact?: boolean; source?: string }) {
  return (
    <a className={compact ? styles.storeCompact : styles.storeButton} href={APP_STORE_URL} target="_blank" rel="noopener noreferrer" onClick={() => trackAppDownload(source)} aria-label="Bizim 6'ncı Kuyumculuk uygulamasını App Store'dan indir">
  ...
  return (
    <section className={styles.inlineSection} aria-labelledby="mobile-app-title">
      ...
      <div className={styles.inlineCopy}>
        <span className={styles.eyebrow}><Sparkles /> Cebinizdeki kuyumcu</span>
        <h2 id="mobile-app-title">Altının ışıltısı artık her an yanında.</h2>
        <p>Yeni koleksiyonları keşfet, favorilerini kaydet ve güvenli alışveriş deneyimini 6&apos;ncı Kuyumculuk uygulamasında yaşa.</p>
      ...
      <div className={styles.phoneStage} aria-hidden="true">
        ...
        <div className={styles.phone}><div className={styles.phoneTop} /><AppIcon /><b>Bizim 6&apos;ncı</b><small>Mücevher dünyası cebinde</small><div className={styles.phoneCard}>Yeni koleksiyon<br/><strong>Şimdi keşfet</strong></div></div>
      </div>
    </section>
  );
}
  ...
  return createPortal(<>
    ...
    {showLaunch ? <div className={styles.launchBackdrop} onMouseDown={(e) => { if (e.target === e.currentTarget) dismissLaunch(); }}>
      <section className={styles.launch} role="dialog" aria-modal="true" aria-labelledby="app-launch-title">
        ...
        <div className={styles.launchVisual}><span className={styles.launchHalo} /><div className={styles.launchPhone}><AppIcon /><b>Bizim 6&apos;ncı</b><small>Kuyumculuk deneyimi cebinde</small></div></div>
        <div className={styles.launchCopy}><span className={styles.eyebrow}><Sparkles /> Uygulamamız yayında</span><h2 id="app-launch-title">Mücevher dünyamızı cebinde taşı.</h2><p>Seçkin koleksiyonlara daha hızlı ulaş, favorilerini yanında tut ve uygulamaya özel fırsatları kaçırma.</p><ul><li><Check /> Koleksiyonlara hızlı erişim</li><li><Check /> Favori ürünlerini kolayca takip</li><li><Check /> Güvenli ve akıcı alışveriş</li></ul><StoreButton source="launch_popup" /><button type="button" className={styles.later} onClick={dismissLaunch}>Şimdilik web sitesinde devam et</button></div>
      </section>
    </div> : null}
  </>, document.body);
}
