...
const FALLBACK_PRODUCT_LOGO = "/logo.png";
...
export default function AdminProductEdit({ params }: { params: { id: string } }) {
  ...
  function buildSeoTitle(form: ProductForm, loc: "tr" | "en" = "tr") {
    ...
    const suffix = loc === "en" ? "6'ncı Jewelry" : "6'ncı Kuyumculuk";
  ...
  function buildSeoDescription(form: ProductForm, loc: "tr" | "en" = "tr") {
    ...
    if (loc === "en") {
      return truncateSeo(
        `${title} model with premium craftsmanship, secure shopping experience, fast shipping and 6'ncı Jewelry assurance.`,
        155
      );
    }
    ...
    return truncateSeo(
      `${title} modeli ${[karat, gram]
        .filter(Boolean)
        .join(", ")} özellikleriyle 6'ncı Kuyumculuk güvencesiyle sunulur. Güvenli alışveriş, hızlı kargo ve zarif paketleme.`,
      155
    );
  }
      ...
      {activeTab === "media" ? (
        <section className={s.card}>
          ...
          <div className={s.mt12}>
            ...
            <div className={s.fieldHint}>
              Ürün görseli eklenmezse sistem otomatik bu görseli kullanır. Örn: /logo.png
            </div>
          </div>
          <IconPackPicker
            label="Kart İkonu"
