...
import MarketplaceAdminHome from "@/components/admin/MarketplaceAdminHome";
...
function LegacyAdminHomePage() {
  ...
  useEffect(() => {
    ...
    if (!CATALOG_RESET_MODE) unsubs.push(
      onSnapshot(
    ...
    <main className={s.page}>
      ...
      <header className={s.hero}>
        <div className={s.heroLeft}>
          <div className={s.eyebrow}>PAZAR. MARKETPLACE OPERATIONS</div>
          <h1 className={s.heroTitle}>Süper Admin Merkezi</h1>
          <p className={s.heroText}>
            Satıcı, katalog, sipariş ve destek akışını tek merkezden yönet.
            Yeni PAZAR. kataloğu sıfırdan ve kontrollü biçimde büyüyor.
          </p>

          <div className={s.heroActions}>
            <Link className={s.primaryBtn} href="/admin/sellers">Satıcılar</Link>
            <Link className={s.softBtn} href="/admin/orders">Siparişler</Link>
            <Link className={s.softBtn} href="/admin/products">Katalog</Link>
      ...
      <section className={s.quickActionsBoard}>
        <QuickAction href="/admin/products" title="Yeni Katalog" text="Sıfır katalog durumunu gör" />
        <QuickAction href="/admin/seller-products" title="Ürün Onayları" text="İşletme ürünlerini değerlendir" />
        ...
        <>
          ...
          <section className={s.grid2}>
            <div className={s.card}>
              ...
              {appointments.length === 0 ? <div className={s.empty}>Henüz randevu talebi yok.</div> : <div className={s.feedList}>{appointments.slice(0, 6).map((a) => <div className={s.feedItem} key={a.id}><div className={s.feedMain}><div className={s.feedTitle}>{a.fullName || "Müşteri"}</div><div className={s.feedText}>{a.interest || "Özel mücevher danışmanlığı"} • {a.appointmentDate || "—"} {a.timeSlot || ""}</div></div><div className={s.feedMeta}><span className={normStatus(a.status) === "new" ? s.badgeWarn : normStatus(a.status) === "cancelled" ? s.badgeBad : s.badgeOk}>{normStatus(a.status) === "new" ? "Yeni" : normStatus(a.status) === "confirmed" ? "Onaylı" : normStatus(a.status) === "completed" ? "Tamamlandı" : normStatus(a.status) === "cancelled" ? "İptal" : "İşlemde"}</span><span className={s.tableSub}>{fmtDate(toDate(a.createdAt))}</span></div></div>)}</div>}
            </div>
            <div className={s.card}>
              ...
              <div className={s.noteBox}>Mobil uygulama kampanya alanları yeni PAZAR. marka akışına hazırlanıyor.</div>
            </div>
          </section>
        </>
      ) : null}
      ...
      <section className={s.quickLinksSection}>
        <div className={s.quickCard}>
          <div className={s.quickTitle}>Marketplace kataloğu</div>
          <div className={s.quickText}>Satıcı, ürün onayı, kategori ve vitrin akışı.</div>
          <div className={s.quickActions}>
            <Link href="/admin/products">Ürünler</Link>
            <Link href="/admin/categories">Kategoriler</Link>
            <Link href="/admin/sellers">Satıcılar</Link>
            <Link href="/satici/basvuru">Başvuru sihirbazı</Link>
          </div>
        </div>
...
export default function AdminHomePage() {
  return CATALOG_RESET_MODE ? <MarketplaceAdminHome /> : <LegacyAdminHomePage />;
}
