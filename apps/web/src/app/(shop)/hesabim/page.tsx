...
export default function HesabimPage() {
  ...
  const customerCollection = CATALOG_RESET_MODE ? "marketplace_customers" : "users";
  ...
  useEffect(() => {
    ...
    if (CATALOG_RESET_MODE && !["profile", "addresses", "security"].includes(qTab)) {
      setTab("profile");
      return;
    }
...
if (
  ...
  useEffect(() => {
    ...
    const profRef = doc(db, customerCollection, uid);
  ...
  consentApproved: toBool(d.consentApproved),
    ...
    const addrRef = collection(db, customerCollection, uid, "addresses");
...
const unsubOrders = CATALOG_RESET_MODE ? (() => {
  setOrders([]);
  setOBusy(false);
  return () => {};
})() : onSnapshot(
  ...
  }, [customerCollection, db, user, isRealUser]);
  ...
  try {
    await setDoc(
      doc(db, customerCollection, user.uid),
      {
        firstName,
        lastName,
        phone,
...
const payload = {
  ...
  ...(editing.id === "new" ? { createdAt: new Date().toISOString() } : {}),
    ...
    try {
      if (editing.id === "new") {
        const ref = await addDoc(collection(db, customerCollection, uid, "addresses"), payload);
        if (payload.isDefault) await ensureOnlyOneDefault(ref.id);
      } else {
        await setDoc(doc(db, customerCollection, uid, "addresses", editing.id), payload, { merge: true });
  ...
  async function ensureOnlyOneDefault(defaultId: string) {
    ...
    await setDoc(
      doc(db, customerCollection, uid, "addresses", defaultId),
    ...
    await Promise.all(
      addresses
        ...
        .map((a) =>
          setDoc(
            doc(db, customerCollection, uid, "addresses", a.id),
    ...
    await setDoc(doc(db, customerCollection, uid), { defaultAddressId: defaultId }, { merge: true });
  }
  ...
  async function removeAddress(id: string) {
    ...
    setAMsg(null);
    try {
      await deleteDoc(doc(db, customerCollection, user.uid, "addresses", id));
      if (profile.defaultAddressId === id) {
        await setDoc(doc(db, customerCollection, user.uid), { defaultAddressId: "" }, { merge: true });
      }
