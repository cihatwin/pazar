...
export async function generateMetadata(): Promise<Metadata> {
  if (CATALOG_RESET_MODE) {
    return {
      title: { default: "PAZAR.", template: "%s | PAZAR." },
  ...
  return {
    ...
    title: {
      ...
      template: seo.meta.titleTemplate || "Altıncı Kuyumculuk",
    },
    ...
    openGraph: {
      ...
      siteName: seo.jsonld.organizationName || seo.meta.publisher || "Altıncı Kuyumculuk",
    ...
    icons: {
      icon: [
        ...
        { url: "/brand-icon-1024.png", type: "image/png", sizes: "1024x1024" },
      ],
...
export async function generateViewport(): Promise<Viewport> {
  if (CATALOG_RESET_MODE) return { themeColor: "#11140f" };
...
export default async function RootLayout({
  ...
  const metaPixelId = CATALOG_RESET_MODE
    ? ""
    : (tracking.meta.enabled ? tracking.meta.pixelId : "") || String(process.env.NEXT_PUBLIC_META_PIXEL_ID || "").trim();
  ...
  const useGtm = !CATALOG_RESET_MODE && !!gtmId;
  const useGa = !CATALOG_RESET_MODE && !!gaId;
  const useGads = !CATALOG_RESET_MODE && !!gadsId && !useGtm;

  const baseUrl = resolveBaseUrl(seo);

  const organizationJsonLd =
    !CATALOG_RESET_MODE && seo.jsonld.enabled
      ? {
        "@context": "https://schema.org",
        "@type": "Organization",
        name: seo.jsonld.organizationName || "Altıncı Kuyumculuk",
  ...
  return (
    <html lang="tr">
      <head>
        ...
        {!CATALOG_RESET_MODE ? <meta name="apple-itunes-app" content="app-id=6760553574" /> : null}
      ...
      `}
          ...
          <Script
            ...
            dangerouslySetInnerHTML={{
              __html: JSON.stringify({
                "@context": "https://schema.org",
                "@type": "WebSite",
                name: seo.jsonld.organizationName || "6'ncı Kuyumculuk",
                alternateName: (seo.meta.brandAliases || "")
                  .split(",")
                  .map((x) => x.trim())
                  .filter(Boolean),
        ...
        {/* LocalBusiness (JewelryStore) JSON-LD — Yerel arama ve Google Maps */}
        {!CATALOG_RESET_MODE && seo.jsonld.enabled ? (
          <Script
            ...
            dangerouslySetInnerHTML={{
              __html: JSON.stringify({
                ...
                address: {
                  "@type": "PostalAddress",
                  streetAddress: "Cumhuriyet, Belediye Cd. No:9",
