...
export async function POST(req: NextRequest) {
  try {
    ...
    const db = adminDb();
    const ipHash = createHash("sha256").update(`${clientIp(req)}:${process.env.APPOINTMENT_RATE_SALT || "6nci"}`).digest("hex");
    ...
    await db.collection("mail").add({
      to: process.env.APPOINTMENT_NOTIFICATION_EMAIL || "info@6nci.com",
      message: {
