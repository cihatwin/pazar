...
function pickTitle(product: ProductDoc, id: string): string {
    return cleanText(product.title || product.name, `6'ncı Kuyumculuk Ürünü ${id}`);
}
...
function pickDescription(product: ProductDoc, title: string): string {
    ...
    return `${title} modeli, 6'ncı Kuyumculuk güvencesiyle sunulan zarif ve modern kuyumculuk ürünüdür.`;
}
...
function pickCategory(product: ProductDoc): string {
    ...
    return cleanText(product.categoryName, "Mücevherat");
}
