...
const CATALOG_RESET_MODE = process.env.NEXT_PUBLIC_CATALOG_RESET_MODE !== "false";
const LEGACY_COMMERCE_API_PREFIXES = [
  "/api/admin/categories",
  "/api/admin/desktop/products",
  "/api/admin/desktop/orders",
...
const LIMITS: { pattern: string; limit: number; windowMs: number }[] = [
  ...
  // Satıcı başvurusu — hesap başına sunucuda ayrıca doğrulanır
  { pattern: "/api/seller/apply", limit: 5, windowMs: 10 * 60_000 },
...
export function middleware(req: NextRequest) {
  ...
  if (CATALOG_RESET_MODE && LEGACY_COMMERCE_API_PREFIXES.some((prefix) => path.startsWith(prefix))) {
    return NextResponse.json(
      { ok:false, items:[], error:"Eski ticaret veri kaynağı PAZAR. reset modunda kapalı.", code:"LEGACY_DATA_ISOLATED" },
