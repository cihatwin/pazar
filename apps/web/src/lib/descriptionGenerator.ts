 ...
 * ──────────────────────────────────────────────────────
 * Template-based product description generator for jewelry e-commerce.
...
export function detectProductType(p: Record<string, unknown>): ProductType {
  ...
  // Then check specific jewelry types
...
const SHORT_TEMPLATES: Record<ProductType, ((f: ProductFacts) => string)[]> = {
  bileklik: [
    ...
    (f) => `Günlük şıklığınıza değer katan ${f.title}.${f.gram ? ` ${f.gram} gram.` : ""} 6'ncı Kuyumculuk güvencesiyle.`,
  ...
  yuzuk: [
    ...
    (f) => `Zarafet ve anlam bir arada: ${f.title}.${f.ayar ? ` ${f.ayar}.` : ""} 6'ncı Kuyumculuk kalitesi.`,
  ...
  genel: [
    (f) => `${f.title} — 6'ncı Kuyumculuk'un özenle seçilmiş koleksiyonundan.${f.ayar ? ` ${f.ayar}.` : ""}`,
...
const LONG_PARA1: Record<ProductType, ((f: ProductFacts) => string)[]> = {
  ...
  genel: [
    ...
    (f) => `${f.title}, modern çizgileri ve kaliteli işçiliğiyle 6'ncı Kuyumculuk'un özenle hazırlanmış koleksiyonundan.`,
  ],
};
...
const LONG_PARA3_CTA: string[] = [
  "Minimal çizgileri ve dengeli görünümü sayesinde hediye alternatifi olarak da tercih edilebilir. 6'ncı Kuyumculuk güvencesiyle güvenli ödeme, hızlı kargo ve kolay iade avantajlarıyla satışa sunulur.",
  "Sevdiklerinize anlamlı bir hediye arayanlar için de ideal bir seçenektir. 6'ncı Kuyumculuk'un güvenli alışveriş ortamında sipariş verebilir, hızlı teslimat imkanından yararlanabilirsiniz.",
  "Özel paketleme seçeneğiyle hediye olarak da sunulabilir. 6'ncı Kuyumculuk güvencesiyle güvenli ödeme, hızlı kargo ve müşteri memnuniyeti önceliğiyle teslim edilir.",
  "Kendinize veya sevdiklerinize özel bir hediye olarak tercih edebilirsiniz. 6'ncı Kuyumculuk; sertifikalı ürün, güvenli ödeme ve hızlı teslimat avantajlarıyla hizmetinizdedir.",
];
